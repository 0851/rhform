(function(g,f){typeof exports==='object'&&typeof module!=='undefined'?f(exports,require('react')):typeof define==='function'&&define.amd?define(['exports','react'],f):(g=g||self,f(g.ReactHeroForm={},g.React));}(this,(function(exports, React){'use strict';var React__default='default'in React?React['default']:React;/**
 ...
 * // => false
 */
var isArray = Array.isArray;/** Detect free variable `global` from Node.js. */
...
var getPrototype = overArg(Object.getPrototypeOf, Object);/* Built-in method references for those with the same name as other `lodash` methods. */
...
var Set = getNative(root, 'Set');/* Built-in method references that are verified to be native. */

var WeakMap = getNative(root, 'WeakMap');/** `Object#toString` result references. */
...
function isEmpty(value) {
  ...
  return true;
}const FormStoreContext = /*#__PURE__*/React__default.createContext({});function defaultValueGetter(e, store, props) {
...
function FormField(props) {
  ...
  const {
    store
  } = React.useContext(FormStoreContext);
  React.useEffect(() => {
    (store === null || store === void 0 ? void 0 : store.addName) && (store === null || store === void 0 ? void 0 : store.addName(name));

    if (rule) {
      let rules = Array.isArray(rule) ? rule : [rule];
      rules.forEach(rule => {
        store === null || store === void 0 ? void 0 : store.addRule(name, rule);
      });
    }

    return () => {
      (store === null || store === void 0 ? void 0 : store.removeName) && (store === null || store === void 0 ? void 0 : store.removeName(name));

      if (rule) {
        let rules = Array.isArray(rule) ? rule : [rule];
        rules.forEach(rule => {
          store === null || store === void 0 ? void 0 : store.removeRule(name, rule);
        });
      }
    };
  }, [name]);
  ...
  if (!store) {
    return /*#__PURE__*/React__default.createElement(as, newProps, children);
  }
  ...
  let onChange = React.useCallback(e => {
  ...
  newProps.onChange = onChange; // let [s, sets] = useState('')
  // newProps.value = s
  // newProps.onChange = (e: any) => {
  //   sets(e.target.value)
  // }

  return /*#__PURE__*/React__default.createElement(React__default.Fragment, null, /*#__PURE__*/React__default.createElement(as, newProps, children), store.isLoading(name) && loadingAs ? loadingAs : undefined);
...
function _extends() {
  ...
  const classNames = [classes.field, inline ? classes.inline : '', required ? classes.required : '', error ? classes.error : '', error && errorClassName ? errorClassName : '', className ? className : ''].filter(n => !!n).join(' ');
  const labelStyle = {
  ...
  }, otherProps), !!label && /*#__PURE__*/React__default.createElement("div", {
  ...
  }, child), !!error ? /*#__PURE__*/React__default.createElement("div", {
    className: classes.message
  }, error) : undefined), !!suffix && /*#__PURE__*/React__default.createElement("div", {
  ...
  return /*#__PURE__*/React__default.createElement(FormStoreContext.Provider, {
    value: {
      store,
      opts
    }
  }, !as ? /*#__PURE__*/React__default.createElement(React__default.Fragment, null, children) : as === 'form' ? /*#__PURE__*/React__default.createElement('form', {
    className: classNames,
    onSubmit: newOnSubmit,
    onReset: newOnReset,
    ...options
  }, children) : /*#__PURE__*/React__default.createElement(as, {
    className: classNames,
    ...options
  }, children));
}
...
const validator = {
  'password'(v, othername, {
    data
  }) {
    if (isString(v) && isString(othername)) {
      return v === get(data, othername);
    }
...
function getValidatorFn(validators, name, rule) {
  let rules = [];
...
const FormStore = (_temp = _class = class FormStore {
  constructor(store, changed, rules = {}) {
    this.store = store;
    ...
    _defineProperty(this, "validStacks", void 0);

    _defineProperty(this, "validLoadings", void 0);

    _defineProperty(this, "validate", debounce(async (name, refresh, silent) => {
      ...
      try {
        ...
        let message = isString(error) ? error : error === null || error === void 0 ? void 0 : error.message;
  ...
  async fnTimeOut(value, fn, time) {
    let res = PromiseTimeout(fn, time).catch(e => {
      throw e;
    });
    res.id = Math.random();
    res.value = value;
  ...
  async runLast(name, v, validators) {
      ...
      setTimeout(async () => {
        ...
        if (getLast() === last) {
          ...
          if (res === true) {
            ...
            let message = isString(res) ? res : res === null || res === void 0 ? void 0 : res.message;
  ...
  async validByName(value, validators) {
    try {
      await Promise.all(validators.map(async v => {
        try {
          let res = await v.validator(value, v.opt, {
            data: this.store,
            rules: this.rules,
            form: this
          });
...
}, _defineProperty(_class, "validator", validator), _temp);function useFormStore(values, rules) {
  let [store, changeStore] = React.useState(values); //只在第一次初始化,其他时候不做初始化操作

  let control = React.useMemo(() => {
    return new FormStore(store, changeStore, rules);
  }, [false]);
  return [control, store, changeStore];
}exports.Form=Form;exports.FormField=FormField;exports.FormItem=FormItem;exports.FormStore=FormStore;exports.FormStoreContext=FormStoreContext;exports.useFormStore=useFormStore;exports.validator=validator;Object.defineProperty(exports,'__esModule',{value:true});})));//# sourceMappingURL=index.js.map
